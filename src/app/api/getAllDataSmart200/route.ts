...
export async function GET(req: Request) {
  try {
    ...
    // - if limit=auto OR (date-filter present AND no numeric limit provided): pull all matches up to MAX_AUTO_LIMIT
    // - else if numeric limit provided: respect it (bounded by MAX_AUTO_LIMIT)
    // - else (no date filter): default 5000 (bounded)
    let effectiveLimit: number;
    if (userLimitStr === "auto" || (hasDateFilter && userLimitStr === "")) {
      effectiveLimit = Math.min(cnt, MAX_AUTO_LIMIT);
    ...
    const [rowsRaw] = await pool.query<RowDataPacket[]>(
      `SELECT * FROM \`${table}\`${whereSql} ORDER BY id ${order} LIMIT ?`,
      [...params, effectiveLimit]
    );

    // Fallback to latest N if filter returned nothing
    let rows: RowDataPacket[] = rowsRaw;
    let usedFallback = false;
    if (!Array.isArray(rows) || rows.length === 0) {
      const [fallbackRows] = await pool.query<RowDataPacket[]>(
    ...
    const limitedNote =
      cnt > effectiveLimit
        ? `Note: ${cnt} rows matched. Export limited to ${effectiveLimit} row(s).`
        : undefined;
    ...
    const combinedNote = [
      usedFallback
        ...
        : "",
      limitedNote || "",
      hasDateFilter && userLimitStr === ""
        ? `Tip: limit was set automatically to include all rows in your date range (up to ${MAX_AUTO_LIMIT}).`
        : "",
    ]
      .filter(Boolean)
      .join("  ");
