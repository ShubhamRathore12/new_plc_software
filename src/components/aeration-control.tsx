...
export default function Home({ data, devices, heat,title ,formatValue}: any) {
  ...
  const isHeating = title !== "AERATION W/O HEATING";
      ...
      <div className="w-full max-w-5xl bg-white rounded-xl shadow-lg p-6">
        ...
        <div className="relative w-full h-[600px]">
          <svg width="100%" height="100%" viewBox="0 0 800 600">
            ...
            {/* Silo */}
            <rect x="50" y="100" width="60" height="150" fill="url(#siloGradient)" filter="url(#shadow)" />
            <rect x="50" y="250" width="60" height="50" fill="#6B7280" filter="url(#shadow)" />
            ...
            {isHeating && (
              <>
                <rect x="150" y="100" width="50" height="100" fill="url(#htrGradient)" filter="url(#shadow)" />
                <path
                ...
                <text x="540" y="120" fontSize="14" fontWeight="400" fill="#4B5563" textAnchor="middle">{deltaT} °C</text>
              </>
            )}
            ...
            {/* Blower */}
            <circle cx="450" cy="400" r="35" fill="#3B82F6" filter="url(#shadow)" />
            <path d="M450 365 Q460 375, 450 385 Q460 395, 450 405 Q440 395, 450 385 Q440 375, 450 365 Z"
              fill="#FFFFFF" stroke="#1F2937" strokeWidth="1" transform="rotate(45 450 385)" />
            <path d="M450 365 Q460 375, 450 385 Q460 395, 450 405 Q440 395, 450 385 Q440 375, 450 365 Z"
              fill="#FFFFFF" stroke="#1F2937" strokeWidth="1" transform="rotate(-45 450 385)" />
            <text x="450" y="450" fontSize="12" fontWeight="700" fill="#1F2937" textAnchor="middle">BLOWER</text>

            {/* T2 Sensor */}
            <rect x="510" y="140" width="60" height="30" fill="#BFDBFE" rx="5" filter="url(#shadow)" />
            <text x="540" y="150" fontSize="14" fontWeight="700" fill="#1F2937" textAnchor="middle">T2</text>
            <text x="540" y="165" fontSize="14" fontWeight="400" fill="#4B5563" textAnchor="middle"> {formatValue(data?.AI_AMBIANT_TEMP || data?.AMBIENT_AIR_TEMP_T2, "°C")}</text>

            {/* Set Duration (only for heating) */}
            {isHeating && (
              <>
                <text x="540" y="180" fontSize="14" fontWeight="700" fill="#1F2937" textAnchor="middle">Set Duration</text>
                <text x="540" y="200" fontSize="14" fontWeight="400" fill="#4B5563" textAnchor="middle">{HEATING_MODE_Set_Run_Duration || "N/A"} h</text>
              </>
            )}
            ...
            {/* Blower % */}
            <rect x="420" y="410" width="30" height="20" fill="#E5E7EB" rx="3" filter="url(#shadow)" />
            <text x="435" y="425" fontSize="12" fontWeight="400" fill="#4B5563" textAnchor="middle">{formatValue(data?.Value_to_Display_EVAP_ACT_SPEED || data?.BLOWER_RPM, "%")}</text>
