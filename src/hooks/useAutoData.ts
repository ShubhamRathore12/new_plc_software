...
export const useAutoData = (autoType: string) => {
  ...
  const fetchData = async () => {
    ...
    if (!table) {
      ...
      if (!isMachineRunning) {
      console.log("Machine is not running, setting data to null");
      setData([]);
      setIsConnected(isInternetConnected);
      setError(null);
      return;
    }
  ...
  const getDeviceStatus = () => {
    ...
    return {
      isMachineRunning: deviceStatus?.machineStatus ?? false,
      isInternetConnected: deviceStatus?.internetStatus ?? false,
      isCoolingWorking: deviceStatus?.hasNewData ?? false,
      statusKey,
      deviceStatus
    };
  };
  ...
  return {
    ...
    retryCount,
    formatValue,
    getDeviceStatus, // Added this helper function
    ...getDeviceStatus() // Spread the status properties for direct access
  };
};